	...
	do {
		...
		if (pDataFileBuffer) {
			...
			*pFilelen = j;
			...
			printf("ReadFileECC Request Memory Failed\n");
			ret = -2;
			break;
		}
	} while (0);
...
int CreateECDSAKey(const char *pFilePath)
{
	...
	do 
	{
		...
		{
			ret = -5;
			printf("CreateECDSAKey--Request Memory Failed\n");
			break;
		}
		...
		{
			ret = -6;
			printf("BIO Create File%s Failed\n",pFileAllPath);
			break;
		}
		...
		{
			ret = -7;
			printf("PEM_write_bio_EC_PUBKEY Write to key File%s Failed\n",pFileAllPath);
			break;
		}
		...
		{
			ret = -8;
			printf("BIOCreate File%s Failed\n",pFileAllPath);
			break;
		}
		...
		{
			ret = -9;
			printf("PEM_write_bio_EC_PUBKEY Write key File%s Failed\n",pFileAllPath);
			break;
		}
...
int ECDSASignToBuffer(const char *szPrivateKeyPath,char *szBufferData,const int nBufferData,const char *szLicenceBuffer,unsigned int *pLicencelen)
{
	...
	do 
	{
		...
		if (!ec_key)
		{
			...
			printf("Read Key From File %s FAiled\n",szPrivateKeyPath);
			break;
		}
...
int My_ECDSASignBufferBase16ToLicenceFile(char *szPrivateKeyPath,char *szBufferData,int nBufferData,char *szLicenceBuf, int *nLicLen)
{
	int nRet = 0;
	//char szLicenceBuffer[512] = {};

	nRet = ECDSASignToBuffer(szPrivateKeyPath,szBufferData,nBufferData,szLicenceBuf, (unsigned int*)nLicLen);
	if (1 == nRet)
	{
		for (int i = 0; i < *nLicLen; i++) {
			printf("[%02X] ", szLicenceBuf[i]&0xff);
			if ( (i+1) % 16 == 0) {	
				printf("\n");
			}
		}
		...
		int  szOff = 0;
		for (int i = 0; i < *nLicLen; i++) {
			szOff += sprintf(szTmp + szOff, "%02X", szLicenceBuf[i]&0xff);
		}
		printf("license is %s\n", szTmp);
		strcpy(szLicenceBuf, szTmp);
	}
